...
suite("Extension Test Suite", () => {
	...
	test("Configuration settings should be accessible", () => {
		...
		assert.ok(
			typeof config.get("autoLocale") === "boolean",
			"autoLocale should be a boolean",
		)
		assert.ok(
			typeof config.get("dayVariable") === "number",
			"dayVariable should be a number",
		)
		assert.ok(
			typeof config.get("nightVariable") === "number",
			"nightVariable should be a number",
		)
